      ...
      switch (Ec.Etype)
      {
         ...
         case EC_ERR_TYPE_EMERGENCY:
         {
            sprintf(estring, "%s EMERGENCY slave:%d error:%4.4x\n",
                    timestr, Ec.Slave, Ec.msg.ErrorCode);
            break;
         }
         case EC_ERR_TYPE_PACKET_ERROR:
         {
            sprintf(estring, "%s PACKET slave:%d index:%4.4x.%2.2x error:%d\n",
                    timestr, Ec.Slave, Ec.Index, Ec.SubIdx, Ec.msg.ErrorCode);
            break;
         }
         ...
         case EC_ERR_TYPE_SOE_ERROR:
         {
            sprintf(estring, "%s SoE slave:%d IDN:%4.4x error:%4.4x %s\n",
                    timestr, Ec.Slave, Ec.Index, (unsigned)Ec.AbortCode, ec_soeerror2string(Ec.msg.ErrorCode));
            break;
         }
         case EC_ERR_TYPE_MBX_ERROR:
         {
            sprintf(estring, "%s MBX slave:%d error:%4.4x %s\n",
                    timestr, Ec.Slave, Ec.msg.ErrorCode, ec_mbxerror2string(Ec.msg.ErrorCode));
            break;
         }
         default:
         {
